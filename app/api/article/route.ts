...
import { assertPublicUrl, BlockedHostError } from "@/lib/net";
...
export async function GET(request: Request) {
  ...
  /**
   * The public web only. Every fetch below this line goes through the same
   * guard (lib/net.ts) — including redirects, which is how the old check
   * here was got round — but rejecting it up front turns what would be a
   * 502 from somewhere deep in extraction into a clear 400.
   */
  try {
    ...
    return NextResponse.json(
      {
        error:
          error instanceof BlockedHostError ? error.message : "That URL cannot be fetched",
      },
      { status: 400 },
    );
  }
